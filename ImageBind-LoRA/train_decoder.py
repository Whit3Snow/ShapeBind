...
from utils import gaussian_diffusion
...
class DecoderTrain(L.LightningModule):
    def __init__(self, lr=5e-4, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42, 
                 ...
                 cond_drop_prob=0.1, heads=16, init_scale=0.25, input_channels=6, layers=24,
                 n_ctx=1024, output_channels=12, time_token_cond=True, width=1024,
        ...
        self.model = PointDiffusionTransformer(device="cpu", dtype=torch.float32, input_channels=input_channels, output_channels=output_channels,
                                               n_ctx=n_ctx, width=width, layers=layers, heads=heads, init_scale=init_scale, time_token_cond=time_token_cond)
        # self.upsample = UpsamplePointDiffusionTransformer()
