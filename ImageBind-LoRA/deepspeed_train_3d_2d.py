...
class ImageBindTrain(L.LightningModule):
    def __init__(self, lr=5e-4, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42,
        ...
        for modality_trunk in self.model.modality_trunks.vision.children():
            modality_trunk.requires_grad_(False)
        for modality_postprocessor in self.model.modality_postprocessors.vision.children():
            modality_postprocessor.requires_grad_(False)
        for modality_head in self.model.modality_heads.vision.children():
            modality_head.requires_grad_(False)
            # final_layer = list(modality_head.children())[-1]
            # final_layer.requires_grad_(False)
...
def parse_args():
    ...
    # parser.add_argument("--datasets", type=str, nargs="+", default=["crosspoint", "shapetalk"], choices=["crosspoint", "dreambooth", "shapetalk"],
    #                     help="Datasets to use for training and validation")
    parser.add_argument("--datasets", type=str, nargs="+", default=["crosspoint"], choices=["crosspoint", "dreambooth", "shapetalk"],
                        help="Datasets to use for training and validation")
...
if __name__ == "__main__":
    ...
    # Load datasets
    if "crosspoint" in args.datasets:
        from datasets.crosspoint import CrossPointDataset, CustomBatchSampler
        train_datasets.append(CrossPointDataset(
            split="train",
        ...
        test_datasets.append(CrossPointDataset(
            split="test",
            sample_points_num = args.sample_points_num,
            # device=args.device,
            transform=ContrastiveTransformations(contrast_transforms, n_views=2 if args.self_contrast else 1)
    ...
    if args.full_model_checkpointing:
        checkpointing = {"enable_checkpointing": args.full_model_checkpointing,
                         "callbacks": [ModelCheckpoint(monitor="val_loss", dirpath=args.full_model_checkpoint_dir,
                                                       filename="shapebind-{epoch:02d}-{val_loss:.2f}",
