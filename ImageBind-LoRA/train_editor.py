...
class ChangeIt3M(L.LightningModule):
    def __init__(self, lr=5e-4, lr_patience=10, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42,
        ...
        # Load full pretrained ImageBind model - Shape - Text
        # self.model = imagebind_model.imagebind_huge(pretrained=True, device=device, model_path="/root/volume/.checkpoints/deepspeed_3d_text_full_1024_v1/imagebind-epoch=57-val_loss=1.37.ckpt/checkpoint/mp_rank_00_model_states.pt", deepspeed=True)
        # self.model = imagebind_model.imagebind_huge(pretrained=True, model_path="/root/volume/.checkpoints/full_1024/last.ckpt")
        
        # Freeze Shape

        # for modality_preprocessor in self.model.modality_preprocessors.shape.children():
        #     modality_preprocessor.requires_grad_(False)
        # for modality_trunk in self.model.modality_trunks.shape.children():
        #     modality_trunk.requires_grad_(False)
        # for modality_postprocessor in self.model.modality_postprocessors.shape.children():
        #     modality_postprocessor.requires_grad_(False)
        # for modality_head in self.model.modality_heads.shape.children():
        #     modality_head.requires_grad_(False)

        # Freeze Text

        # for modality_preprocessor in self.model.modality_preprocessors.text.children():
        #     modality_preprocessor.requires_grad_(False)
        # for modality_trunk in self.model.modality_trunks.text.children():
        #     modality_trunk.requires_grad_(False)
        # for modality_postprocessor in self.model.modality_postprocessors.text.children():
        #     modality_postprocessor.requires_grad_(False)
        # for modality_head in self.model.modality_heads.text.children():
        #     modality_head.requires_grad_(False)
        ...
        self_contrast=True
        self.gamma = None
        self.editor = MLP(in_dim, [256, shape_latent_dim, shape_latent_dim, shape_latent_dim], b_norm=True, remove_final_bias=True)
    ...
    def editor_loss(self, batch, mode="train"):
        ...
        edit_loss, batch_accuracy = self.editor_model.single_step_train(feats_a_tensor, feats_b_tensor, self.gamma)
