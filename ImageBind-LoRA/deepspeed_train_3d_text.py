...
class ImageBindTrain(L.LightningModule):
    ...
    def info_nce_loss(self, batch, mode="train"):
        ...
        # get token prediction loss
        # gt_tokens = feats_a["gt_tokens"]
        # tokens = feats_a["tokens"]

        # token_loss = self.model.cel(tokens.reshape(-1, tokens.size(-1)), gt_tokens.reshape(-1,))
        ...
        self.log(mode + "_loss", dual_nll, prog_bar=True,
                 on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, batch_size=self.hparams.batch_size)
        # self.log(mode + "_token_loss", token_loss, prog_bar = True, 
        #          on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, batch_size=self.hparams.batch_size)
