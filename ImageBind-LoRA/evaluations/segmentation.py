...
class Segmentation(L.LightningModule):
    def __init__(self, lr=5e-4, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42,
        ...
        in_dim = 1024
        concat_dim = 1
        self.Seg1 = nn.Sequential(nn.Linear(in_dim, in_dim + in_dim), nn.relu())
        self.Seg2 = nn.Sequential(nn.Linear(in_dim * 2 + concat_dim, in_dim * 3), nn.relu())
        self.Seg3 = nn.Sequential(nn.Linear(in_dim * 3 + concat_dim, in_dim * 4), nn.relu())
        self.Seg4 = nn.Sequential(nn.Linear(in_dim * 4 + concat_dim, 1024))
    ...
    def classification_loss(self, batch, mode="train"):
        ...
        tokens = pc_latent[1:]
        x= self.Seg1(tokens)
        x= self.Seg2(torch.cat(x, cls, dim = 0))
        x= self.Seg3(torch.cat(x, cls, dim = 0))
        x= self.Seg4(torch.cat(x, cls, dim = 0))

        # x => [1024 , m]


        ce_loss = nn.CrossEntropyLoss()
        val_loss = ce_loss(x, labels)
...
if __name__ == "__main__":
    ...
    if "shapetalk" in args.datasets:
        ...
        train_datasets.append(EmbeddingDataset(
            '../../volume/embeddings/embeddings-edit.npz', split="train",
        ...
        test_datasets.append(EmbeddingDataset(
            '../../volume/embeddings/embeddings-edit.npz', split="test",
            sample_points_num = args.sample_points_num
            # device=args.device,
            # transform=ContrastiveTransformations(contrast_transforms, n_views=2 if args.self_contrast else 1)
        ))
    ...
    # Train dataset
    model = Segmentation(max_epochs=args.max_epochs, batch_size=args.batch_size, lr=args.lr,
