...
class Classification(L.LightningModule):
    def __init__(self, lr=5e-4, lr_patience=10, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42,
        ...
        self.model = imagebind_model.imagebind_huge(pretrained=True, model_path="/root/volume/.checkpoints/deepspeed_3d_text_full_1024_v1/imagebind-epoch=57-val_loss=1.37.ckpt/checkpoint/mp_rank_00_model_states.pt", deepspeed=True)
        in_dim = 384
        self.cls = nn.Linear(in_dim, 55)
    ...
    def classification_loss(self, batch, mode="train"):
        pc_latent, labels = batch


        feats_a = self.model({ModalityType.SHAPE: pc_latent})
        feats_a_tensor = feats_a[ModalityType.SHAPE]
        ...
        # [64, 17, 384]
        outputs = self.cls(pc_latent[:, 0, ...])
        ce_loss = nn.CrossEntropyLoss()

        val_loss = ce_loss(outputs, labels.squeeze(1))
        
        preds = torch.argmax(outputs, dim=1)
        
        self.log(mode + "_acc_top1", (preds==labels).float().mean(), prog_bar=True,
                     on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, batch_size=self.hparams.batch_size)
        self.log(mode + "_loss", val_loss, prog_bar=True,
                     on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, batch_size=self.hparams.batch_size)
        
        return val_loss

    def training_step(self, batch, batch_idx):
        return self.classification_loss(batch, mode="train")

    def validation_step(self, batch, batch_idx):

        self.classification_loss(batch, mode="val")
...
def parse_args():
    ...
    parser.add_argument("--datasets", type=str, nargs="+", default=["shapenet"], choices=["shapenet"],
    ...
    parser.add_argument("--lr_patience", type=int, default=5e-6, help="Learning rate sheduler patience")
...
if __name__ == "__main__":
    ...
    # Load datasets
    if "shapenet" in args.datasets:
        import sys
        sys.path.append('/root/ShapeBind/ImageBind-LoRA/datasets')

        from shapenet import ShapeNetEmbDataset
        
        train_datasets.append(ShapeNetEmbDataset(
            '/root/volume/embeddings/embeddings-cls-train.npz', split="train",
        ...
        test_datasets.append(ShapeNetEmbDataset(
            '/root/volume/embeddings/embeddings-cls-test.npz', split="test",
            # sample_points_num = args.sample_points_num
            # device=args.device,
            # transform=ContrastiveTransformations(contrast_transforms, n_views=2 if args.self_contrast else 1)
        ))
    ...
    # Train dataset
    model = Classification(max_epochs=args.max_epochs, batch_size=args.batch_size, lr=args.lr, lr_patience=args.lr_patience,
