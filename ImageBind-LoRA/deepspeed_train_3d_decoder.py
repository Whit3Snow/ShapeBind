...
class Decoder(L.LightningModule):
    def __init__(self, max_epochs=350, batch_size=32, lr=0.0005, lr_patience=10,
                    num_workers=4, encoder_latent_dim=1024):
        super().__init__()

        self.save_hyperparameters()

        self.ae_model = PointcloudAutoencoder(encoder_latent_dim)
    ...
    def decoder_loss(self, batch, mode="train"):

        z_shape, pointcloud = batch
        
        chamfer_dist = self.ae_model.single_step_train(pointcloud, z_shape)

        self.log(mode + "_loss", chamfer_dist, prog_bar=True,
                     on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, batch_size=self.hparams.batch_size)

        return chamfer_dist

    def training_step(self, batch, batch_idx):
        return self.decoder_loss(batch, mode="train")

    def validation_step(self, batch, batch_idx):
        self.decoder_loss(batch, mode="val")
...
def parse_args():
    ...
    parser.add_argument("--datasets", type=str, nargs="+", default=["embedding"], choices=["shapetalk", "embedding"],
    ...
    # sample_points_num 1024 in encoder but 4096 in decoder
    parser.add_argument("--sample_points_num", type=int, default=4096,
    ...
    parser.add_argument("--encoder_latent_dim", type=int, default = 1024)
...
if __name__ == "__main__":
    ...
    # Train dataset
    model = Decoder(max_epochs=args.max_epochs, batch_size=args.batch_size, lr=args.lr, lr_patience = args.lr_patience,
                    num_workers=args.num_workers, encoder_latent_dim=args.encoder_latent_dim)
